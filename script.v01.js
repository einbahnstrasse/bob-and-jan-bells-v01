...
document.getElementById('enable-button').addEventListener('click', async () => {
  ...
  const enableGyro = () => {
    window.addEventListener('devicemotion', (event) => {
      ...
      document.getElementById('alphaLabel').textContent = `PITCH (rotate left to right): ${a.toFixed(2)}`;
      document.getElementById('betaLabel').textContent = `TRIGGER (tilt front to back): ${b.toFixed(2)}`;
      document.getElementById('gammaLabel').textContent = `VOLUME (lean side to side): ${g.toFixed(2)}`;
